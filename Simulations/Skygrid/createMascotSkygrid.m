...
for i = 1 : length(tree_files)
   ...
   % coalescing
    ...
    for tr = 1 : 1    
        ...
        fprintf(g,'\t\t\t<parameter id="NeLog.state0" name="stateNode" dimension="1">0</parameter>\n');
        fprintf(g,'\t\t\t<parameter id="NeLog.state1" name="stateNode" dimension="1">0</parameter>\n');        
...
%         fprintf(g,'\t\t\t\t<distribution spec=''beast.math.distributions.Prior'' x="@NeLog.state0">\n');
...
%         fprintf(g,'\t\t\t\t</distribution>\n');
        ...
        fprintf(g,'\t\t\t\t<distr spec="beast.math.distributions.Exponential"  mean="0.5"/>\n');
        fprintf(g,'\t\t\t\t</distribution>\n');


        fprintf(g,'\t\t\t\t<distribution spec=''beast.mascotskyline.skyline.LogSmoothingPrior'' NeLog="@NeLog.state0">\n');
        fprintf(g,'\t\t\t\t\t<distr spec="beast.math.distributions.Normal"  mean="0" sigma="0.5"/>\n');
        fprintf(g,'\t\t\t\t\t<initialDistr spec="beast.math.distributions.Normal"  mean="0" sigma="1.0"/>\n');
        fprintf(g,'\t\t\t\t</distribution>\n');
        fprintf(g,'\t\t\t\t<distribution spec=''beast.mascotskyline.skyline.LogSmoothingPrior'' NeLog="@NeLog.state1">\n');
        fprintf(g,'\t\t\t\t\t<distr spec="beast.math.distributions.Normal"  mean="0" sigma="0.5"/>\n');
        fprintf(g,'\t\t\t\t\t<initialDistr spec="beast.math.distributions.Normal"  mean="0" sigma="1.0"/>\n');
        fprintf(g,'\t\t\t\t</distribution>\n');
% 
...
%         fprintf(g,'\t\t\t\t</distribution>\n');
        ...
        fprintf(g,'\t\t\t\t\t<dynamics id="piecewiseConstant" spec="beast.mascotskyline.dynamics.DynamicEffectivePopulationSizes" forwardsMigration="@migration" dimension="0" typeTrait="@typeTraitSet.t" rateShifts="@rateshifts.t">\n');
        fprintf(g,'\t\t\t\t\t\t<NeDynamics id="skygrid.state0" spec="beast.mascotskyline.parametric.StructuredSkygrid" NeLog="@NeLog.state0">\n');
        fprintf(g,'\t\t\t\t\t\t\t<rateShifts id="rateshifts.t" spec="beast.mascotskyline.skyline.RateShifts">\n');
        fprintf(g,'\t\t\t\t\t\t\t\t<parameter id="rateshifts" name="absoluteRateShifts">%s</parameter>\n', sprintf('%f ', 1:1:10));
        fprintf(g,'\t\t\t\t\t\t\t</rateShifts>\n');
        fprintf(g,'\t\t\t\t\t\t</NeDynamics>\n');
        fprintf(g,'\t\t\t\t\t\t<NeDynamics id="skygrid.state1" spec="beast.mascotskyline.parametric.StructuredSkygrid" NeLog="@NeLog.state1" rateShifts="@rateshifts.t"/>\n');
        fprintf(g,'\t\t\t\t\t</dynamics>\n');        
        fprintf(g,'\t\t\t\t</distribution>\n');
        fprintf(g,'\t\t\t</distribution>\n');
        fprintf(g,'\t\t</distribution>\n');

        fprintf(g,'\t\t<operator id="AMVGoperator" spec="beast.mascotskyline.operators.AdaptiveMultivariateGaussianOperator" windowSize="0.1" weight="25.0">\n');
		fprintf(g,'\t\t\t<parameter idref="NeLog.state0"/>\n');
        fprintf(g,'\t\t\t<parameter idref="NeLog.state1"/>\n');
        fprintf(g,'\t\t\t<parameter idref="migration"/>\n');
        fprintf(g,'\t\t\t<logTransform id="logTransform" estimate="false" spec="parameter.BooleanParameter">false false true</logTransform>\n');
		fprintf(g,'\t\t</operator>\n');


% 		fprintf(g,'\t\t<operator id="NenullScaler.1" spec="RealRandomWalkOperator" windowSize="0.1" parameter="@NeLog.state0" weight="3.0"/>\n');
% 		fprintf(g,'\t\t<operator id="NenullScaler.2" spec="RealRandomWalkOperator" windowSize="0.1" parameter="@NeLog.state1" weight="3.0"/>\n');
% 
%         
%         fprintf(g,'\t\t<operator id="MigrationScaler" spec="ScaleOperator" scaleFactor="0.8" optimise="true" parameter="@migration" weight="10.0"/>\n');
        ...
        fprintf(g,'\t\t\t<log idref="NeLog.state0"/>\n');
        fprintf(g,'\t\t\t<log idref="NeLog.state1"/>\n');
        fprintf(g,'\t\t</logger>\n');
