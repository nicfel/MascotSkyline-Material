    ...
    for tr = 1 : 1    
        ...
        while ~feof(g)
            ...
            elseif contains(line ,'insert_rate_shifts')
                fprintf(f, strrep(line, 'insert_rate_shifts', sprintf('%f ', 1:10)));
            else 
                fprintf(f, line);
            end
        end
...
%% make the same, but only for one state
...
for i = 1 : length(tree_files)
   ...
   % coalescing
    ...
    for tr = 1 : 1    
        ...
        flog = strrep(tree_files(i).name,'master.tree',sprintf('%donestate',tr));
        fname = sprintf('xmls/%s.xml',flog);
        f = fopen(fname,'w');
        while ~feof(g)
            line = fgets(g);
            ...
            elseif contains(line, 'insert_states')       
                ...
                for j = 1 : length(leafnames)
                    tmp1 = strsplit(leafnames{j},'_');
                    tstates = [tstates sprintf(',%s=state0',leafnames{j})];
                end
                fprintf(f, strrep(line, 'insert_states', strrep(tstates, 'rem,','')));
            elseif contains(line ,'insert_rate_shifts')
                fprintf(f, strrep(line, 'insert_rate_shifts', sprintf('%f ', 1:10)));
            elseif contains(line, '<parameter id="migrationRatesSkyline.t:H3N2"')
                fprintf(f, strrep(line, '1.0','0.0'));
            elseif contains(line, ' <prior id="migrationRatesSkylinePrior.t:H3N2"')
                fgets(g);fgets(g);fgets(g);fgets(g);
            elseif contains(line, '<f idref="migrationRatesSkyline.t:H3N2"/>')
            else 
                fprintf(f, line);
            end
        end
